...
from utils import get_training_keypoints
...
class Audio2Keypoint(nn.Module):
    ...
    def _train_D(self, audio_spect, real_pose):
        ...
        Returns
        -------
        real_score  : torch.tensor of shape (B, 16)
                      Realism score assigned by discriminator for real pose
        fake_score  : torch.tensor of shape (B, 16)
        ...
        real_score = self.discriminator(real_pose, self.keypoints) #(B,16)
        fake_score = self.discriminator(fake_pose, self.keypoints)
    ...
    def _train_G(self, audio_spect, real_pose):
        ...
        fake_score = self.discriminator(fake_pose, self.keypoints)
        return img_enc_piv, fake_pose, real_enc, fake_enc, fake_score
    ...
    def _train_E(self, audio_spect, real_pose):
        ...
        real_enc = self.encoder(real_pose) #(B,32)
        fake_enc = self.encoder(fake_pose) #(B,32)
        fake_score = self.discriminator(D_fake_pose, self.keypoints)
        return img_enc_piv, fake_pose, real_enc, fake_enc, fake_score
    ...
    def _predict(self, audio_spect, real_pose):
        ...
        fake_enc = self.encoder(fake_pose) #(B,32)
        fake_score = self.discriminator(D_fake_pose, self.keypoints)
        return fake_pose, fake_score, fake_enc, img_enc_piv
