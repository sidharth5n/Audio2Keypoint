...
class ImageEncoderPV(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv2 = ConvLayer(128, 128, 4, 2, 1, '1D')
...
class AudioEncoder(nn.Module):
    ...
    def __init__(self):
        ...
        self.downsampling_blocks1to4 = nn.Sequential(ConvLayer(in_channels, 64, 3, 1, 1, '2D'),
                                                     ...
                                                     ConvLayer(
                                                         256, 256, 3, 1, 1, '2D'),
                                                     ConvLayer(256, 256, (3, 8), 1, 0, '2D'))  # padding such that same size
        # self.downsampling_blocks5to10 = nn.ModuleList([nn.Sequential(ConvLayer(in_channels, 256, 3, 1, 1, '1D'),
        #                                                              ConvLayer(256, 256, 3, 1, 1, '1D')),
        #                                                ConvLayer(in_channels, 256, 4, 2, padding, '1D'), #padding such that same size
        #                                                ConvLayer(in_channels, 256, 4, 2, padding, '1D'), #padding such that same size,
        ...
        #                                                ConvLayer(in_channels, 256, 4, 2, padding, '1D') #padding such that same size
        #                                               ])
        self.downsampling_blocks10_downsampling_false = ConvLayer(
            416, 256, 3, 1, 1, '1D')
        self.downsampling_blocks5_downsampling_true = ConvLayer(
            256, 256, 4, 2, 1, '1D')
        self.downsampling_blocks5_downsampling_false = ConvLayer(
            256, 256, 3, 1, 1, '1D')
    ...
    def forward(self, audio, pose, img_enc_pv, img_enc_piv):
        ...
        # print(x.shape)
        # .squeeze(2) # check dim
        x = F.interpolate(x, (pose.shape[2], 1),
        ...
        seventh_block = self.downsampling_blocks5_downsampling_true(
            sixth_block)

        eight_block = self.downsampling_blocks5_downsampling_true(
            seventh_block)
        ...
        tenth_block = torch.cat([tenth_block, img_enc_pv, img_enc_piv], dim=1)
        tenth_block = self.downsampling_blocks10_downsampling_false(
            tenth_block)
        print(tenth_block.shape)
        ...
        seventh_block = self.downsampling_blocks5_downsampling_false(
            seventh_block)
        ...
        decoder_input = self.downsampling_blocks5_downsampling_false(
            fifth_block)
        print("decoder_input")
...
class Discriminator(nn.Module):
    def __init__(self, d_input='pose'):
        ...
        if d_input == 'motion':
            ...
            self.motion_or_pose = lambda x: torch.cat(
                [x, to_motion_delta(x)], dim=1)  # check dim
        n_downsampling = 2
        in_channels = 134
        m = 0
        n = 0
        ...
        layers.append(ConvLayer(in_channels, 64, 4, 2, 1, '1D', False))
        # layers.append(PrintSize())
        ...
        for i in range(1, n_downsampling+1):
            ...
            if(i != n_downsampling):
                # layers.append(PrintSize())
                layers.append(ConvLayer(64*m, 64*nf_mult, 4, 2, 1, '1D'))
                layers.append(PrintSize())
                # padding such that same size
        layers.append(nn.ConstantPad1d((1, 2), 0))
        layers.append(ConvLayer(64*m, 64*nf_mult, 4, 1, 0, '1D'))

        layers.append(PrintSize())
        layers.append(nn.ConstantPad1d((1, 2), 0))
        # padding such that same size
        layers.append(nn.Conv1d((64*(2**i)), 1, 4, 1, 0))
        layers.append(PrintSize())
...
class Generator(nn.Module):
    ...
    def forward(self, audio, pose, image, image_enc_piv):
        ...
        audio_enc = self.audio_encoder(
            audio_input, pose, image_enc_pv, image_enc_piv)
