...
def decode_pose_normalized_keypoints(encoded_keypoints, shift, speaker, scale_to_jon=True):
    #encoded_keypoints = np.reshape(encoded_keypoints, (-1, 2, 68))
    #encoded_keypoints[:, :, 27] = 0.
    #encoded_keypoints = np.reshape(encoded_keypoints, (-1, 136))
    denormalaized = de_normalize_relative_keypoints(encoded_keypoints, speaker, scale_to_jon)
    denormalaized = np.reshape(denormalaized, (-1, 2, 68))
